...
MCP Connect for WordPress transforma cualquier instalación de WordPress en un servidor **MCP** (Model Context Protocol) compatible con el transporte **Streamable HTTP**, protegido con **OAuth 2.1** y **PKCE**. Copia una URL en tu agente de IA y este podrá leer y gestionar tu contenido usando solo los permisos que autorices.
...
* Sistema de permisos por scopios (`categoria:modo`) gestionado desde Ajustes → MCP Connect for WordPress.
...
1. Sube la carpeta `mcp-connect-wp` a `/wp-content/plugins/` (o instala el ZIP desde Plugins → Añadir nuevo → Subir plugin).
2. Activa el plugin desde el menú Plugins.
3. Ve a Ajustes → **MCP Connect for WordPress**.
...
Los que se marquen en Ajustes → MCP Connect for WordPress → Permisos y, además, los que tu usuario de WordPress tenga. Por defecto la lectura y escritura están activas y el borrado desactivado.
...
En Ajustes → MCP Connect for WordPress → Conexiones autorizadas puedes ver qué clientes se han conectado, con qué usuario y con qué permisos, y revocarlas en cualquier momento.
