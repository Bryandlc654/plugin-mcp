	...
	ready( function () {
		...
		if ( testBtn && result && window.MCPConnectAdmin ) {
			testBtn.addEventListener( 'click', function () {
				...
				result.className = 'mcp-connect-wp-test-result';
				result.textContent = '';
				...
				fetch( window.MCPConnectAdmin.rest + '/health', {
					...
					.then( function ( res ) {
						if ( res.ok && res.data.status === 'ok' ) {
							if ( res.data.mcp ) {
								result.className = 'mcp-connect-wp-test-result ok';
								result.textContent = window.MCPConnectAdmin.i18n.ok + ' · MCP ' + res.data.version;
							} else {
								result.className = 'mcp-connect-wp-test-result fail';
		...
		var copyContainers = document.querySelectorAll( '.mcp-connect-wp-copy[data-copy]' );
		...
		var checkAll = document.getElementById( 'mcp-connect-wp-check-all' );
		if ( checkAll ) {
			checkAll.addEventListener( 'change', function () {
				var boxes = document.querySelectorAll( '#mcp-connect-wp-scopes input[type=checkbox]:enabled' );
