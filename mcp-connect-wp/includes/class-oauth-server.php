...
final class OAuth_Server {
	...
	public function handle_authorize() {
		if ( ! Rate_Limiter::allowed( $this->url->get_client_ip(), 'authorize', 30 ) ) {
			return $this->html( 429, __( 'Too many attempts. Please try again later.', 'mcp-connect-wp' ) );
		}
		...
		if ( $_SERVER['REQUEST_METHOD'] === 'POST' ) {
			...
			if ( ! wp_verify_nonce( $nonce, 'mcp_connect_authorize' ) ) {
				return $this->html( 403, __( 'Security check failed. Please reload the page and try again.', 'mcp-connect-wp' ) );
			}
	...
	private function validate_authorize_request( array $params ) {
		if ( 'code' !== $params['response_type'] ) {
			return array( 'ok' => false, 'client' => null, 'redirect_uri' => null, 'error_page' => __( 'Only the "code" response type is supported.', 'mcp-connect-wp' ) );
		}

		$client = $this->registration->resolve_client( $params['client_id'] );
		if ( ! $client ) {
			return array( 'ok' => false, 'client' => null, 'redirect_uri' => null, 'error_page' => __( 'Unknown client. Please reconnect your AI agent.', 'mcp-connect-wp' ) );
		}

		$redirect_uri = $params['redirect_uri'];
		if ( ! in_array( $redirect_uri, $client->redirect_uris, true ) ) {
			return array( 'ok' => false, 'client' => $client, 'redirect_uri' => null, 'error_page' => __( 'The redirect URI does not match the registered client.', 'mcp-connect-wp' ) );
		}
	...
	private function show_consent() {
		...
		$data = array(
			...
			'client_name'  => $validation['client']->client_name ? $validation['client']->client_name : ( isset( $validation['client']->is_cimd ) ? $params['client_id'] : __( 'Unnamed client', 'mcp-connect-wp' ) ),
	...
	private function process_consent() {
		...
		if ( 'cancel' === $params['action'] ) {
			...
			return $this->html( 400, __( 'The request could not be processed.', 'mcp-connect-wp' ) );
		}
		...
		if ( ! $inserted ) {
			return $this->html( 500, __( 'Could not create the authorization code.', 'mcp-connect-wp' ) );
		}
	...
	public function handle_token() {
		if ( ! Rate_Limiter::allowed( $this->url->get_client_ip(), 'token', 30 ) ) {
			return $this->json( 429, array( 'error' => 'too_many_requests', 'error_description' => __( 'Too many attempts.', 'mcp-connect-wp' ) ), array( 'Retry-After' => '60' ) );
		}
		...
		return $this->json( 400, array( 'error' => 'unsupported_grant_type', 'error_description' => __( 'The grant type is not supported.', 'mcp-connect-wp' ) ) );
	}
	...
	private function grant_authorization_code( array $params, $client ) {
		...
		if ( '' === $code || '' === $verifier ) {
			return $this->json( 400, array( 'error' => 'invalid_request', 'error_description' => __( 'Missing code or code_verifier.', 'mcp-connect-wp' ) ) );
		}

		$row = $this->tokens->consume_code( $code );
		if ( ! $row || $row->client_id !== $client->client_id ) {
			return $this->json( 400, array( 'error' => 'invalid_grant', 'error_description' => __( 'The authorization code is invalid or has already been used.', 'mcp-connect-wp' ) ) );
		}
		if ( $row->redirect_uri !== $redirect_uri ) {
			return $this->json( 400, array( 'error' => 'invalid_grant', 'error_description' => __( 'The redirect URI does not match the authorization request.', 'mcp-connect-wp' ) ) );
		}

		$normalized_resource = $this->url->normalize_resource( $resource );
		if ( null === $normalized_resource ) {
			return $this->json( 400, array( 'error' => 'invalid_request', 'error_description' => __( 'The resource is not valid.', 'mcp-connect-wp' ) ) );
		}
		if ( $row->resource && strcasecmp( rtrim( $row->resource, '/' ), rtrim( $normalized_resource, '/' ) ) !== 0 ) {
			...
			return $this->json( 400, array( 'error' => 'invalid_grant', 'error_description' => __( 'The resource does not match the authorization request.', 'mcp-connect-wp' ) ) );
		}

		if ( ! Crypto::pkce_verify( $verifier, $row->code_challenge ) ) {
			$this->tokens->revoke_client_user( $row->client_id, (int) $row->user_id );
			return $this->json( 400, array( 'error' => 'invalid_grant', 'error_description' => __( 'Invalid PKCE code verifier.', 'mcp-connect-wp' ) ) );
		}
	...
	private function grant_refresh_token( array $params, $client ) {
		...
		$resource = isset( $params['resource'] ) ? (string) $params['resource'] : '';
		if ( '' === $refresh ) {
			return $this->json( 400, array( 'error' => 'invalid_request', 'error_description' => __( 'Missing refresh_token.', 'mcp-connect-wp' ) ) );
		}
