	...
	private function initialize( $id, array $params ) {
		...
		return JsonRpc::success(
			...
			array(
				...
				'serverInfo'      => array(
					'name'    => 'MCP Connect for WordPress',
					'version' => MCP_CONNECT_VERSION,
				),
				'instructions'    => sprintf(
					__( 'You are connected to the WordPress site %s via MCP Connect for WordPress. Use the available wp_* tools to help the user manage their WordPress content. Respect the user\'s capabilities and always ask before destructive actions.', 'mcp-connect-wp' ),
					get_bloginfo( 'name' )
				),
			)
		);
