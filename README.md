...
> El código fuente se encuentra en `mcp-connect/`. La documentación técnica completa está en [docs/](docs/) y el diseño en [ARCHITECTURE.md](ARCHITECTURE.md).
...
cp -r mcp-connect /path/to/wp-content/plugins/

# 2. Activa en el panel: Plugins → MCP Connect
# 3. Ajustes → MCP Connect → revisa Permisos y copia la "URL del servidor MCP"
```

## Conectar un agente

Pega la URL del servidor MCP (`https://tusitio.com/wp-json/mcp-connect/v1/mcp`) como URL de servidor MCP en tu cliente.
...
```json
{
  "mcpServers": {
    "wordpress": {
      "url": "https://tusitio.com/wp-json/mcp-connect/v1/mcp"
    }
  }
}
```
...
```
mcp-connect/
  mcp-connect.php              Bootstrap, constantes, autoloader
...
php -l $(find mcp-connect -name '*.php') > /dev/null
...
GPL-2.0-or-later. Ver [mcp-connect/readme.txt](mcp-connect/readme.txt).
# plugin-mcp
