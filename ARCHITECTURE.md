...
- A single MCP endpoint is exposed: `https://example.com/wp-json/wp-mcp-connect/v1/mcp`
...
The plugin is namespaced `WPMCPConnect\`. Files:

```
wp-mcp-connect/
├── wp-mcp-connect.php                     # bootstrap, constants, autoloader, hooks
...
1. **Protected resource metadata — RFC 9728**
   ...
   - `GET /wp-json/wp-mcp-connect/v1/.well-known/oauth-protected-resource` (path-insertion
...
2. **Authorization server metadata — RFC 8414**
   ...
   - `GET /wp-json/wp-mcp-connect/v1/.well-known/oauth-authorization-server`
...
3. **401 challenge (mandatory per MCP spec)**
   ...
   WWW-Authenticate: Bearer resource_metadata="https://…/wp-json/wp-mcp-connect/v1/.well-known/oauth-protected-resource",
                     authorization_server="https://…/wp-json/wp-mcp-connect/v1/.well-known/oauth-authorization-server",
...
- `POST /wp-json/wp-mcp-connect/v1/oauth/register`
...
client → POST /wp-json/wp-mcp-connect/v1/mcp (Authorization: Bearer …)
```
...
| table                      | purpose                                      |
| -------------------------- | -------------------------------------------- |
| `{prefix}wp_mcp_connect_clients`       | registered OAuth clients (secrets hashed)    |
| `{prefix}wp_mcp_connect_codes`         | one-time PKCE authorization codes            |
| `{prefix}wp_mcp_connect_tokens`        | access + refresh tokens (SHA-256, families)  |
| `{prefix}wp_mcp_connect_authorizations`| user↔client↔scope grants (for revoke/UI)     |
| `{prefix}wp_mcp_connect_logs`          | audit log                                    |
| `{prefix}wp_mcp_connect_rate`          | IP rate limiting                             |
