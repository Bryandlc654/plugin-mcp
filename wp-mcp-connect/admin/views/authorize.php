<?php
// Consent screen rendered by WPMCPConnect\OAuth_Server. $data is sanitized by the caller.
...
<title><?php echo esc_html( sprintf( __( 'Connect %s', 'wp-mcp-connect' ), $data['site_name'] ) ); ?></title>
<style>
...
<div class="card">
	<div class="brand">
		...
		<p><?php echo esc_html( __( 'Conectar agente de IA', 'wp-mcp-connect' ) ); ?></p>
	</div>
	<div class="user">
	...
	<div class="agent">
		<?php echo esc_html( __( 'Un agente de IA solicita acceso a', 'wp-mcp-connect' ) ); ?>:
		<strong><?php echo esc_html( $data['client_name'] ); ?></strong>
	</div>
	<div class="scopes">
		<h2><?php echo esc_html( __( 'Permisos', 'wp-mcp-connect' ) ); ?></h2>
		<ul>
	...
	<?php if ( $data['redirect_uri'] ) : ?>
		<p class="hint"><?php printf( esc_html__( 'Al autorizar redirigiremos a: %s', 'wp-mcp-connect' ), '<code style="word-break:break-all">' . esc_html( $data['redirect_uri'] ) . '</code>' ); ?></p>
	<?php endif; ?>
	<form method="post" action="<?php echo esc_url( \WPMCPConnect\Plugin::instance()->url->oauth_endpoint( 'authorize' ) ); ?>">
		<input type="hidden" name="_mcp_nonce" value="<?php echo esc_attr( wp_create_nonce( 'wp_mcp_connect_authorize' ) ); ?>">
		...
		<div class="actions">
			<button type="submit" name="deny" value="1" class="cancel" onclick="this.form.action.value='cancel'"><?php echo esc_html__( 'Cancelar', 'wp-mcp-connect' ); ?></button>
			<button type="submit" class="approve"><?php echo esc_html__( 'Autorizar', 'wp-mcp-connect' ); ?></button>
		</div>
	</form>
	<p class="foot"><?php echo esc_html( __( 'Autorizado por', 'wp-mcp-connect' ) ); ?> <strong><?php echo esc_html( $data['site_name'] ); ?></strong> · WP MCP Connect</p>
</div>
</body>
</html>