...
return array(
	...
	array(
		...
		'description' => __( 'Lists the site\'s post categories.', 'wp-mcp-connect' ),
	...
	array(
		...
		'description' => __( 'Creates a new post category.', 'wp-mcp-connect' ),
		...
		'inputSchema' => array(
			...
			'properties'           => array(
				...
				'parent'      => array( 'type' => 'integer', 'description' => __( 'Parent category ID.', 'wp-mcp-connect' ) ),
		...
		'permission'  => function ( $args, $actor ) {
			if ( ! Permissions::user_has( $actor['user_id'], 'manage_categories' ) ) {
				return array( 'allowed' => false, 'code' => 'insufficient_permissions', 'message' => __( 'Managing categories requires the manage_categories capability.', 'wp-mcp-connect' ) );
			}
	...
	array(
		...
		'description' => __( 'Updates an existing category (name, slug, description, parent).', 'wp-mcp-connect' ),
		'scope'       => 'taxonomies:write',
		'category'    => 'taxonomies',
		'mode'        => 'write',
		'inputSchema' => array(
	...
	array(
		...
		'description' => __( 'Deletes a category (the default "Uncategorized" category cannot be deleted). Destructive action.', 'wp-mcp-connect' ),
		...
		'handler'     => function ( $args ) {
			...
			if ( ! $term ) {
				throw new Tool_Error( 'not_found', __( 'Category not found.', 'wp-mcp-connect' ) );
			}
			if ( 1 === (int) $term->term_id ) {
				throw new Tool_Error( 'protected', __( 'The default category cannot be deleted.', 'wp-mcp-connect' ) );
			}
			...
			if ( ! $result ) {
				throw new Tool_Error( 'deletion_failed', __( 'The category could not be deleted.', 'wp-mcp-connect' ) );
			}
	...
	array(
		'name'        => 'wp_list_tags',
		'description' => __( 'Lists the site\'s post tags.', 'wp-mcp-connect' ),
		'scope'       => 'taxonomies:read',
		'category'    => 'taxonomies',
		'mode'        => 'read',
		'inputSchema' => array(
	...
	array(
		'name'        => 'wp_create_tag',
		'description' => __( 'Creates a new post tag.', 'wp-mcp-connect' ),
		'scope'       => 'taxonomies:write',
		'category'    => 'taxonomies',
		'mode'        => 'write',
		'inputSchema' => array(
		...
		'permission'  => function ( $args, $actor ) {
			if ( ! Permissions::user_has( $actor['user_id'], 'manage_categories' ) ) {
				return array( 'allowed' => false, 'code' => 'insufficient_permissions', 'message' => __( 'Managing tags requires the manage_categories capability.', 'wp-mcp-connect' ) );
			}
			return array( 'allowed' => true );
		},
		'handler'     => function ( $args ) {
	...
	array(
		'name'        => 'wp_update_tag',
		'description' => __( 'Updates an existing tag.', 'wp-mcp-connect' ),
		'scope'       => 'taxonomies:write',
		'category'    => 'taxonomies',
		'mode'        => 'write',
		'inputSchema' => array(
	...
	array(
		'name'        => 'wp_delete_tag',
		'description' => __( 'Deletes a tag. Destructive action.', 'wp-mcp-connect' ),
		'scope'       => 'taxonomies:delete',
		'category'    => 'taxonomies',
		'mode'        => 'delete',
		'inputSchema' => array(
		...
		'handler'     => function ( $args ) {
			...
			if ( ! $term ) {
				throw new Tool_Error( 'not_found', __( 'Tag not found.', 'wp-mcp-connect' ) );
			}
			...
			if ( ! $result ) {
				throw new Tool_Error( 'deletion_failed', __( 'The tag could not be deleted.', 'wp-mcp-connect' ) );
			}
			return array( 'deleted' => true, 'id' => (int) $term->term_id );
		},
	),
