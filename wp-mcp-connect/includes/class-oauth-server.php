...
final class OAuth_Server {
	...
	public function handle_authorize() {
		if ( ! Rate_Limiter::allowed( $this->url->get_client_ip(), 'authorize', 30 ) ) {
			return $this->html( 429, __( 'Too many attempts. Please try again later.', 'wp-mcp-connect' ) );
		}

		if ( $_SERVER['REQUEST_METHOD'] === 'POST' ) {
			$nonce = isset( $_POST['_mcp_nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['_mcp_nonce'] ) ) : '';
			if ( ! wp_verify_nonce( $nonce, 'wp_mcp_connect_authorize' ) ) {
				return $this->html( 403, __( 'Security check failed. Please reload the page and try again.', 'wp-mcp-connect' ) );
			}
	...
	private function validate_authorize_request( array $params ) {
		if ( 'code' !== $params['response_type'] ) {
			return array( 'ok' => false, 'client' => null, 'redirect_uri' => null, 'error_page' => __( 'Only the "code" response type is supported.', 'wp-mcp-connect' ) );
		}

		$client = $this->registration->resolve_client( $params['client_id'] );
		if ( ! $client ) {
			return array( 'ok' => false, 'client' => null, 'redirect_uri' => null, 'error_page' => __( 'Unknown client. Please reconnect your AI agent.', 'wp-mcp-connect' ) );
		}

		$redirect_uri = $params['redirect_uri'];
		if ( ! in_array( $redirect_uri, $client->redirect_uris, true ) ) {
			return array( 'ok' => false, 'client' => $client, 'redirect_uri' => null, 'error_page' => __( 'The redirect URI does not match the registered client.', 'wp-mcp-connect' ) );
		}
	...
	private function show_consent() {
		...
		$data = array(
			...
			'client_name'  => $validation['client']->client_name ? $validation['client']->client_name : ( isset( $validation['client']->is_cimd ) ? $params['client_id'] : __( 'Unnamed client', 'wp-mcp-connect' ) ),
	...
	private function process_consent() {
		...
		if ( 'cancel' === $params['action'] ) {
			...
			return $this->html( 400, __( 'The request could not be processed.', 'wp-mcp-connect' ) );
		}
		...
		if ( ! $inserted ) {
			return $this->html( 500, __( 'Could not create the authorization code.', 'wp-mcp-connect' ) );
		}
	...
	private function render_authorize_html( array $data ) {
		ob_start();
		include WP_MCP_CONNECT_DIR . 'admin/views/authorize.php';
		return ob_get_clean();
	}
	...
	public function handle_token() {
		if ( ! Rate_Limiter::allowed( $this->url->get_client_ip(), 'token', 30 ) ) {
			return $this->json( 429, array( 'error' => 'too_many_requests', 'error_description' => __( 'Too many attempts.', 'wp-mcp-connect' ) ), array( 'Retry-After' => '60' ) );
		}
		...
		return $this->json( 400, array( 'error' => 'unsupported_grant_type', 'error_description' => __( 'The grant type is not supported.', 'wp-mcp-connect' ) ) );
	}
	...
	private function grant_authorization_code( array $params, $client ) {
		...
		if ( '' === $code || '' === $verifier ) {
			return $this->json( 400, array( 'error' => 'invalid_request', 'error_description' => __( 'Missing code or code_verifier.', 'wp-mcp-connect' ) ) );
		}

		$row = $this->tokens->consume_code( $code );
		if ( ! $row || $row->client_id !== $client->client_id ) {
			return $this->json( 400, array( 'error' => 'invalid_grant', 'error_description' => __( 'The authorization code is invalid or has already been used.', 'wp-mcp-connect' ) ) );
		}
		if ( $row->redirect_uri !== $redirect_uri ) {
			return $this->json( 400, array( 'error' => 'invalid_grant', 'error_description' => __( 'The redirect URI does not match the authorization request.', 'wp-mcp-connect' ) ) );
		}

		$normalized_resource = $this->url->normalize_resource( $resource );
		if ( null === $normalized_resource ) {
			return $this->json( 400, array( 'error' => 'invalid_request', 'error_description' => __( 'The resource is not valid.', 'wp-mcp-connect' ) ) );
		}
		if ( $row->resource && strcasecmp( rtrim( $row->resource, '/' ), rtrim( $normalized_resource, '/' ) ) !== 0 ) {
			...
			return $this->json( 400, array( 'error' => 'invalid_grant', 'error_description' => __( 'The resource does not match the authorization request.', 'wp-mcp-connect' ) ) );
		}

		if ( ! Crypto::pkce_verify( $verifier, $row->code_challenge ) ) {
			$this->tokens->revoke_client_user( $row->client_id, (int) $row->user_id );
			return $this->json( 400, array( 'error' => 'invalid_grant', 'error_description' => __( 'Invalid PKCE code verifier.', 'wp-mcp-connect' ) ) );
		}
	...
	private function grant_refresh_token( array $params, $client ) {
		...
		$resource = isset( $params['resource'] ) ? (string) $params['resource'] : '';
		if ( '' === $refresh ) {
			return $this->json( 400, array( 'error' => 'invalid_request', 'error_description' => __( 'Missing refresh_token.', 'wp-mcp-connect' ) ) );
		}
