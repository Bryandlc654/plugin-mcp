...
final class OAuth_Client_Registration {
	...
	public function handle_register( array $params ) {
		if ( ! $this->settings->is_registration_enabled() ) {
			return array(
				'status'  => 403,
				'headers' => array(),
				'body'    => array( 'error' => 'registration_disabled', 'error_description' => __( 'Dynamic client registration is disabled.', 'wp-mcp-connect' ) ),
				'format'  => 'json',
			);
		}

		if ( ! Rate_Limiter::allowed( $this->url->get_client_ip(), 'register', 10 ) ) {
			return $this->error( 429, 'too_many_requests', __( 'Too many registration attempts. Try again later.', 'wp-mcp-connect' ) );
		}

		$redirect_uris = isset( $params['redirect_uris'] ) ? $params['redirect_uris'] : array();
		if ( ! is_array( $redirect_uris ) ) {
			return $this->error( 400, 'invalid_redirect_uri', __( 'redirect_uris must be an array of URIs.', 'wp-mcp-connect' ) );
		}
		if ( empty( $redirect_uris ) ) {
			return $this->error( 400, 'invalid_redirect_uri', __( 'At least one redirect URI is required.', 'wp-mcp-connect' ) );
		}

		$normalized = array();
		foreach ( $redirect_uris as $uri ) {
			if ( ! $this->url->is_valid_redirect_uri( $uri ) ) {
				return $this->error( 400, 'invalid_redirect_uri', __( 'Redirect URIs must be absolute HTTPS URIs, loopback HTTP URIs or custom app schemes without fragments.', 'wp-mcp-connect' ) );
			}
			$normalized[] = $uri;
		}
		...
		foreach ( $grant_types as $grant ) {
			if ( ! in_array( $grant, array( 'authorization_code', 'refresh_token' ), true ) ) {
				return $this->error( 400, 'invalid_client_metadata', __( 'Only authorization_code and refresh_token grants are supported.', 'wp-mcp-connect' ) );
			}
		}
		...
		if ( ! is_array( $response_types ) || array_diff( $response_types, array( 'code' ) ) ) {
			return $this->error( 400, 'invalid_client_metadata', __( 'Only the code response type is supported.', 'wp-mcp-connect' ) );
		}
		...
		if ( ! in_array( $auth_method, array( 'none', 'client_secret_basic', 'client_secret_post' ), true ) ) {
			return $this->error( 400, 'invalid_client_metadata', __( 'Unsupported token endpoint authentication method.', 'wp-mcp-connect' ) );
		}
		...
		if ( array_diff( $parsed_scopes, $grantable ) ) {
			return $this->error( 400, 'invalid_scope', __( 'The requested scope contains scopes that are not supported.', 'wp-mcp-connect' ) );
		}
		...
		if ( ! $inserted ) {
			return $this->error( 500, 'server_error', __( 'Could not store the client registration.', 'wp-mcp-connect' ) );
		}
	...
	private function fetch_client_id_document( $url ) {
		$cache_key = 'wp_mcp_connect_cimd_' . md5( $url );
