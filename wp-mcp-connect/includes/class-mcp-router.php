...
final class MCP_Router {
	...
	public function register_rewrite_rules() {
		add_rewrite_rule( '^\.well-known/oauth-protected-resource/?$', 'index.php?wp_mcp_connect_wk=protected-resource', 'top' );
		add_rewrite_rule( '^\.well-known/oauth-authorization-server/?$', 'index.php?wp_mcp_connect_wk=authorization-server', 'top' );
		add_rewrite_rule( '^\.well-known/openid-configuration/?$', 'index.php?wp_mcp_connect_wk=authorization-server', 'top' );
	}

	public function add_query_vars( $vars ) {
		$vars[] = 'wp_mcp_connect_wk';
		return $vars;
	}

	public function serve_well_known( \WP $wp ) {
		$kind = isset( $wp->query_vars['wp_mcp_connect_wk'] ) ? $wp->query_vars['wp_mcp_connect_wk'] : '';
		if ( ! $kind ) {
			return;
		}
	...
	public function register_rest_routes() {
		...
		register_rest_route( 'wp-mcp-connect/v1', '/health', array( 'methods' => 'GET', 'callback' => array( $this, 'handle_health' ), 'permission_callback' => '__return_true' ) );
		...
		register_rest_route( 'wp-mcp-connect/v1', '/.well-known/oauth-protected-resource', array( 'methods' => 'GET', 'callback' => array( $this, 'route_any' ), 'permission_callback' => '__return_true' ) );
		register_rest_route( 'wp-mcp-connect/v1', '/.well-known/oauth-authorization-server', array( 'methods' => 'GET', 'callback' => array( $this, 'route_any' ), 'permission_callback' => '__return_true' ) );
		register_rest_route( 'wp-mcp-connect/v1', '/.well-known/openid-configuration', array( 'methods' => 'GET', 'callback' => array( $this, 'route_any' ), 'permission_callback' => '__return_true' ) );
	...
	public function handle_health() {
		...
		$body   = array(
			'status'      => 'ok',
			'plugin'      => 'WP MCP Connect',
			'version'     => WP_MCP_CONNECT_VERSION,
	...
	public function handle_admin_rest( \WP_REST_Request $request ) {
		...
		if ( ! wp_verify_nonce( $nonce, $plugin->admin->ajax_nonce_action() ) ) {
			return $this->emit( $this->json_response( array( 'success' => false, 'message' => __( 'Invalid nonce.', 'wp-mcp-connect' ) ), 403 ) );
		}
	...
	private function is_our_route() {
		...
		return ( strpos( $route, 'wp-mcp-connect/v1' ) !== false );
	}
