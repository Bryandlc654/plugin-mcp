...
class Url_Manager {
	...
	public function mcp_endpoint( $trailing = false ) {
		$url = untrailingslashit( get_rest_url( null, 'wp-mcp-connect/v1/mcp' ) );
		return $trailing ? $url . '/' : $url;
	}

	public function oauth_endpoint( $endpoint ) {
		return get_rest_url( null, 'wp-mcp-connect/v1/oauth/' . $endpoint );
	}

	public function well_known_url( $doc ) {
		return untrailingslashit( get_rest_url( null, 'wp-mcp-connect/v1/.well-known/' . $doc ) );
	}
	...
	public function normalize_resource( $resource ) {
		...
		// Legacy bare-namespace root (wp-mcp-connect/v1) still accepted as the resource.
		if ( strcasecmp( $r, untrailingslashit( get_rest_url( null, 'wp-mcp-connect/v1' ) ) ) === 0 ) {
			return $canonical;
		}
		return null;
	}
