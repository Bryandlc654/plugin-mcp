...
return array(
	...
	array(
		'name'        => 'wp_list_pages',
		'description' => __( 'Lists WordPress pages. Use this tool when the user asks to see, list or show their pages (static pages, not blog posts).', 'mcp-connect' ),
		'scope'       => 'pages:read',
		'category'    => 'pages',
		'mode'        => 'read',
		'inputSchema' => array(
			...
			'properties'           => array(
				...
				'parent'  => array( 'type' => 'integer', 'description' => __( 'Parent page ID.', 'mcp-connect' ) ),
		...
		'permission'  => function ( $args, $actor ) {
			...
			if ( ! in_array( $status, array( 'publish', 'any' ), true ) && ! Permissions::user_has( $actor['user_id'], 'edit_pages' ) ) {
				return array( 'allowed' => false, 'code' => 'insufficient_permissions', 'message' => __( 'Non-published pages require the edit_pages capability.', 'mcp-connect' ) );
			}
	...
	array(
		'name'        => 'wp_get_page',
		'description' => __( 'Gets a single WordPress page by ID with its full content.', 'mcp-connect' ),
		'scope'       => 'pages:read',
		'category'    => 'pages',
		'mode'        => 'read',
		'inputSchema' => array(
		...
		'permission'  => function ( $args, $actor ) {
			...
			if ( $post && 'publish' !== $post->post_status && ! Permissions::user_has( $actor['user_id'], 'edit_page', (int) $args['id'] ) ) {
				return array( 'allowed' => false, 'code' => 'insufficient_permissions', 'message' => __( 'You cannot read this unpublished page.', 'mcp-connect' ) );
			}
			return array( 'allowed' => true );
		},
		'handler'     => function ( $args ) {
	...
	array(
		'name'        => 'wp_create_page',
		'description' => __( 'Creates a new WordPress page. Use this tool when the user asks to create a new page. The page can be created as draft, pending, private or published, subject to capabilities.', 'mcp-connect' ),
		'scope'       => 'pages:write',
		'category'    => 'pages',
		'mode'        => 'write',
		'inputSchema' => array(
			...
			'properties'           => array(
				...
				'parent'     => array( 'type' => 'integer', 'description' => __( 'Parent page ID.', 'mcp-connect' ) ),
				'template'   => array( 'type' => 'string', 'description' => __( 'Page template name.', 'mcp-connect' ) ),
				'order'      => array( 'type' => 'integer', 'description' => __( 'Menu order.', 'mcp-connect' ) ),
			),
		...
		'permission'  => function ( $args, $actor ) {
			if ( ! Permissions::user_has( $actor['user_id'], 'edit_pages' ) ) {
				return array( 'allowed' => false, 'code' => 'insufficient_permissions', 'message' => __( 'Creating pages requires the edit_pages capability.', 'mcp-connect' ) );
			}
			...
			if ( in_array( $status, array( 'publish', 'private' ), true ) && ! Permissions::user_has( $actor['user_id'], 'publish_pages' ) ) {
				return array( 'allowed' => false, 'code' => 'insufficient_permissions', 'message' => __( 'The authenticated WordPress user cannot publish pages.', 'mcp-connect' ) );
			}
			return array( 'allowed' => true );
		},
		'handler'     => function ( $args ) {
	...
	array(
		'name'        => 'wp_update_page',
		'description' => __( 'Updates an existing WordPress page.', 'mcp-connect' ),
		'scope'       => 'pages:write',
		'category'    => 'pages',
		'mode'        => 'write',
		'inputSchema' => array(
		...
		'permission'  => function ( $args, $actor ) {
			if ( ! Permissions::user_has( $actor['user_id'], 'edit_page', (int) $args['id'] ) ) {
				return array( 'allowed' => false, 'code' => 'insufficient_permissions', 'message' => __( 'You cannot edit this page.', 'mcp-connect' ) );
			}
			$post = get_post( (int) $args['id'] );
			if ( $post && isset( $args['status'] ) && in_array( $args['status'], array( 'publish', 'private' ), true ) && 'publish' !== $post->post_status && 'private' !== $post->post_status && ! Permissions::user_has( $actor['user_id'], 'publish_pages' ) ) {
				return array( 'allowed' => false, 'code' => 'insufficient_permissions', 'message' => __( 'Publishing pages requires the publish_pages capability.', 'mcp-connect' ) );
			}
			return array( 'allowed' => true );
		},
		'handler'     => function ( $args ) {
	...
	array(
		'name'        => 'wp_delete_page',
		'description' => __( 'Deletes a WordPress page. Moves it to the trash by default or permanently deletes it when force is true. Destructive action.', 'mcp-connect' ),
		'scope'       => 'pages:delete',
		'category'    => 'pages',
		'mode'        => 'delete',
		'inputSchema' => array(
		...
		'permission'  => function ( $args, $actor ) {
			if ( ! Permissions::user_has( $actor['user_id'], 'delete_page', (int) $args['id'] ) ) {
				return array( 'allowed' => false, 'code' => 'insufficient_permissions', 'message' => __( 'You cannot delete this page.', 'mcp-connect' ) );
			}
			return array( 'allowed' => true );
		},
		'handler'     => function ( $args ) {
			...
			if ( ! empty( $args['force'] ) ) {
				...
				if ( ! $result ) {
					throw new Tool_Error( 'deletion_failed', __( 'The page could not be deleted.', 'mcp-connect' ) );
				}
