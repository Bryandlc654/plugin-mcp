...
final class Admin {
	...
	public function load_page() {
		wp_enqueue_style( 'mcp-connect-admin', plugin_dir_url( MCP_CONNECT_FILE ) . 'admin/css/admin.css', array(), MCP_CONNECT_VERSION );
		wp_enqueue_script( 'mcp-connect-admin', plugin_dir_url( MCP_CONNECT_FILE ) . 'admin/js/admin.js', array( 'jquery' ), MCP_CONNECT_VERSION, true );
		wp_localize_script(
			'mcp-connect-admin',
			'MCPConnectAdmin',
			array(
				'nonce' => wp_create_nonce( self::NONCE ),
				'rest'  => esc_url_raw( rest_url( 'mcp-connect/v1' ) ),
				'i18n'  => array(
					'testing' => __( 'Comprobando conexión…', 'mcp-connect' ),
					'ok'      => __( 'Conectado correctamente', 'mcp-connect' ),
					'fail'    => __( 'Error de conexión', 'mcp-connect' ),
				),
			)
		);
	}
	...
	public function render_page() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'No tienes permisos para acceder a esta página.', 'mcp-connect' ) );
		}
		...
		?>
		<div class="wrap mcp-connect-admin">
			<h1><?php echo esc_html__( 'MCP Connect', 'mcp-connect' ); ?></h1>
			<?php $this->render_tabs( $tab ); ?>
			<div class="mcp-connect-content">
				<?php
				if ( $view ) {
					include $view;
				}
	...
	private function render_tabs( $active ) {
		$tabs = array(
			'connect'      => __( 'Conectar', 'mcp-connect' ),
			'permissions'  => __( 'Permisos', 'mcp-connect' ),
			'clients'      => __( 'Conexiones autorizadas', 'mcp-connect' ),
			'logs'         => __( 'Logs', 'mcp-connect' ),
			'diagnostics'  => __( 'Diagnóstico', 'mcp-connect' ),
		);
	...
	private function check_cap_nonce() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'No tienes permisos.', 'mcp-connect' ), 403 );
		}
		if ( ! isset( $_POST['_mcp_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['_mcp_nonce'] ), self::NONCE ) ) {
			wp_die( esc_html__( 'Nonce inválido.', 'mcp-connect' ), 403 );
		}
	}
	...
	public function run_diagnostics() {
		...
		$checks[] = array(
			'name'    => __( 'Conexión HTTPS', 'mcp-connect' ),
			'ok'      => $https,
			'message' => $https
				? __( 'El sitio se sirve por HTTPS.', 'mcp-connect' )
				: __( 'HTTPS no está activo: necesitas HTTPS para que los agentes de IA puedan conectarse.', 'mcp-connect' ),
		);

		$permalinks = get_option( 'permalink_structure' );
		$checks[] = array(
			'name'    => __( 'Enlaces permanentes', 'mcp-connect' ),
			'ok'      => (bool) $permalinks,
			'message' => $permalinks
				? sprintf( __( 'Estructura: %s', 'mcp-connect' ), '<code>' . esc_html( $permalinks ) . '</code>' )
				: __( 'Sin estructura de enlaces permanentes: activa una estructura distinta de "Simple".', 'mcp-connect' ),
		);
		...
		foreach ( array( 'clients', 'codes', 'tokens', 'authorizations', 'logs' ) as $table ) {
			...
			$checks[] = array(
				'name'    => sprintf( __( 'Tabla %s', 'mcp-connect' ), $table ),
				'ok'      => $tables_ok,
				'message' => $tables_ok ? esc_html( $name ) : __( 'No existe. Al activar el plugin se crea automáticamente.', 'mcp-connect' ),
			);
		}

		$checks[] = array(
			'name'    => __( 'Servidor MCP', 'mcp-connect' ),
			'ok'      => $plugin->is_enabled(),
			'message' => $plugin->is_enabled()
				? __( 'Plugin activo y servidor habilitado.', 'mcp-connect' )
				: __( 'El plugin está desactivado.', 'mcp-connect' ),
		);

		$checks[] = array(
			'name'    => __( 'Criptografía', 'mcp-connect' ),
			'ok'      => Crypto::available(),
			'message' => Crypto::available()
				? __( 'random_bytes y hash disponibles.', 'mcp-connect' )
				: __( 'Faltan funciones de criptografía.', 'mcp-connect' ),
		);

		$checks[] = array(
			'name'    => __( 'Registro dinámico', 'mcp-connect' ),
			'ok'      => $plugin->settings->is_registration_enabled(),
			'message' => $plugin->settings->is_registration_enabled()
				? __( 'Los clientes pueden registrarse dinámicamente.', 'mcp-connect' )
				: __( 'Registro dinámico desactivado.', 'mcp-connect' ),
		);

		$checks[] = array(
			'name'    => __( 'WordPress', 'mcp-connect' ),
			'ok'      => version_compare( $wp_version, '6.0', '>=' ),
			'message' => sprintf( __( 'Versión: %s', 'mcp-connect' ), esc_html( $wp_version ) ),
		);
