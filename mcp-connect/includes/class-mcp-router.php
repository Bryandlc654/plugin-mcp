...
final class MCP_Router {
	...
	public function register_rest_routes() {
		...
		register_rest_route( 'mcp-connect/v1', '/health', array( 'methods' => 'GET', 'callback' => array( $this, 'handle_health' ), 'permission_callback' => '__return_true' ) );
		...
		register_rest_route( 'mcp-connect/v1', '/.well-known/oauth-protected-resource', array( 'methods' => 'GET', 'callback' => array( $this, 'route_any' ), 'permission_callback' => '__return_true' ) );
		register_rest_route( 'mcp-connect/v1', '/.well-known/oauth-authorization-server', array( 'methods' => 'GET', 'callback' => array( $this, 'route_any' ), 'permission_callback' => '__return_true' ) );
		register_rest_route( 'mcp-connect/v1', '/.well-known/openid-configuration', array( 'methods' => 'GET', 'callback' => array( $this, 'route_any' ), 'permission_callback' => '__return_true' ) );
	...
	public function handle_admin_rest( \WP_REST_Request $request ) {
		...
		if ( ! wp_verify_nonce( $nonce, $plugin->admin->ajax_nonce_action() ) ) {
			return $this->emit( $this->json_response( array( 'success' => false, 'message' => __( 'Invalid nonce.', 'mcp-connect' ) ), 403 ) );
		}
	...
	private function is_our_route() {
		...
		return ( strpos( $route, 'mcp-connect/v1' ) !== false );
	}
