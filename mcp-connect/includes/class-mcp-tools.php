...
final class MCP_Tools {
	...
	public function call( $name, array $arguments, array $actor ) {
		$def = $this->get( $name );
		if ( ! $def ) {
			return $this->error_result( 'method_not_found', __( 'Tool not found.', 'mcp-connect' ) );
		}
		...
		if ( ! $settings->scope_enabled( $cat, $mode ) ) {
			return $this->error_result( 'tool_disabled', __( 'This tool is disabled by the administrator.', 'mcp-connect' ) );
		}
		if ( ! Permissions::has_scope( $actor['scopes'], $def['scope'] ) ) {
			return $this->error_result( 'insufficient_scope', __( 'The granted scope does not include this tool.', 'mcp-connect' ) );
		}
		...
		if ( ! empty( $def['permission'] ) && is_callable( $def['permission'] ) ) {
			...
			if ( is_array( $check ) && isset( $check['allowed'] ) && ! $check['allowed'] ) {
				$message = isset( $check['message'] ) ? $check['message'] : __( 'You do not have permission to perform this action.', 'mcp-connect' );
		...
		try {
			...
			return $this->error_result( 'internal_error', __( 'An unexpected error occurred while running the tool.', 'mcp-connect' ) );
		}
